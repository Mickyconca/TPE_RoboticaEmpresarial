...
class PomodoroApp:
    def __init__(self, root):
        ...
        self.start_button = tk.Button(self.root, text="Iniciar Pomodoro", command=self.start_pomodoro)
    ...
    def toggle_automatic_mode(self):
        ...
        if self.automatic_mode:
            self.automatic_button.config(text="Detener Pomodoro Automático")
            self.start_pomodoro()
        else:
            self.automatic_button.config(text="Iniciar Pomodoro Automático")
            self.stop_timer()
    ...
    def update_timer(self):
        ...
        if self.remaining_time > 0:
            ...
            if self.mode == "pomodoro":
                self.start_button.config(text="Iniciar Recreo Corto", command=self.start_short_break)
                if self.automatic_mode:
    ...
    def configure_times(self):
        ...
        self.break_time = simpledialog.askinteger("Configurar", "Tiempo de recreo corto (en minutos)",
                                                        minvalue=1) * 60
